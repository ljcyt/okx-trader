...
# ── 三位裁判：打分视角 ──────────────────────────────────────────────
# 历史教训：技术/风控裁判在 18 条提案上极差只有 1.0 分且零否决——
# 闸门退化成资金管理裁判一个人的独角戏。根因是提示词允许"无硬伤→7-8"，
# 模型把"没看出毛病"当成给高分的理由。以下提示词把默认翻转：
# 过闸分（≥7）必须回答"为什么值得现在冒险"，答不出具体证据就是 4-6 分。
JUDGES = [
    {"name": "技术裁判",
     "prompt": ("你是「技术裁判」。审查提案的方向是否与趋势/动量/形态因子自洽，"
               "止损价方向正确、距离合理(0.2%~5%)。打分纪律："
               "顺趋势且指标完全配合、能引用因子报告里的具体数字作最强证据→8-9分；"
               "方向与主趋势相悖→3-4分；理由与数据矛盾→0-2分。"
               "指标有冲突（如金叉但均线交织、多周期方向不一致）→4-6分——"
               "冲突本身就是降分理由，不存在'没有硬伤所以高分'。"
               "给 7 分以上时，你的 concerns 里必须写出那条最强证据的原文数字。")},
    {"name": "风控裁判",
     "prompt": ("你是「风控裁判」。只看风险：止损距离区间、波动极端度(ATR/价格>3%)、"
               "资金费率异常(|费率|>0.1%)、量比异常放大(>3)。打分纪律："
               "所有维度都干净且能逐项引用报告数字确认→8-9分；"
               "任一维度异常→4-6分；危险→0-3分。"
               "'没看到异常'不等于'确认干净'——报告里缺失的数据（如分位不足）"
               "按 6 分处理并写明缺什么。给 7 分以上时必须逐项列出你核对过的维度。")},
    {"name": "资金管理裁判",
     "prompt": ("你是「资金管理裁判」。评估这笔交易值不值得占用风险预算："
               "盈亏比按两个口径都看——离最近结构位的 RR 和离更远结构位的 RR；"
               "近位 RR<1 时这笔交易的现实阻力测试会更早到来，要明确扣分并写明；"
               "账户已有同向暴露（相关标的）要扣分并写明。"
               "打分纪律：机会质量高（近位远位 RR 都达标、无叠加暴露）→8-9分；"
               "平庸（依赖远位目标、或有同向叠加）→4-6分；差→0-4分。"
               "给 7 分以上时必须写出'为什么是现在而不是等更好的位置'。")},
]
